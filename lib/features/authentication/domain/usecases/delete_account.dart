...
class DeleteAccountUsecase extends Usecases<void, String> {
  ...
  FutureEither<void> call(String params) async {
    ...
    return await deleteStorage.fold((failure) async => Left(failure), (
      _,
    ) async {
      final deleteCart = await _cartRepository.deleteCart(params);

      return await deleteCart.fold((failure) async => Left(failure), (_) async {
        final deleteProfile = await _userRepository.deleteUserProfile(params);

        return await deleteProfile.fold((failure) async => Left(failure), (
          _,
        ) async {

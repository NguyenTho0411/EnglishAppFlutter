...
import 'package:flutter_application_1/core/extensions/datetime_extensions.dart';
...
class DailyWordCubit extends Cubit<DailyWordState> {
  ...
  Future<void> reload() async {
    ...
    result.fold((failure) => emit(DailyWordErrorState(failure.message)), (
      list,
    ) async {
      final knowns = sl<SharedPrefManager>().getKnownWords;
      WordEntity? random = list
          .where((e) => !knowns.contains(e.word))
          .toList()
          .getRandom;
      if (random != null) {
